...
import type { MicroserviceConfig } from '../config/microservices';
    ...
    private addMicroserviceRoute(name: string, config: MicroserviceConfig): void {
        ...
        proxy.on('proxyReq', (
            ...
            if (expressReq.body && Object.keys(expressReq.body).length > 0) {
                ...
                // On s'assure que les headers sont bien synchronisés avec le nouveau body
                proxyReq.setHeader('Content-Type', 'application/json');
                proxyReq.setHeader('Content-Length', Buffer.byteLength(bodyData));

                // On ré-injecte le body dans le flux du proxy
        ...
        proxy.on('error', (err, req, res) => {
            ...
            if (!response.headersSent) {
                response.status(503).json({ error: 'Service unavailable', service: name });
            }
        });
